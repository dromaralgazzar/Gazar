...
/* BOOKING HOURS VALIDATION - GAZZAR Dental Clinic
   Working hours: 3:00 PM to 10:00 PM, Friday closed.
*/
(function(){
  ...
  function isValidWorkingTime(timeValue) {
    if (!timeValue) return false;
    return timeValue >= '15:00' && timeValue <= '22:00';
  }
  ...
  dateInput?.addEventListener('change', function(){
    if (isFriday(this.value)) {
      showBookingError('الجمعة إجازة. من فضلك اختار يوم آخر من السبت إلى الخميس.');
      this.value = '';
    }
  });

  timeInput?.addEventListener('change', function(){
    if (this.value && !isValidWorkingTime(this.value)) {
      showBookingError('المواعيد المتاحة من 3:00 مساءً إلى 10:00 مساءً فقط.');
      this.value = '';
    }
  });
  ...
  form.addEventListener('submit', function(e){
    ...
    if (!isValidWorkingTime(timeValue)) {
      e.preventDefault();
      e.stopImmediatePropagation();
      showBookingError('المواعيد المتاحة من 3:00 مساءً إلى 10:00 مساءً فقط.');
      return false;
...
modalForm?.addEventListener('submit',e=>{e.preventDefault();const d=new FormData(modalForm);const msg=`مرحبًا، أريد تأكيد حجز موعد في GAZZAR Dental Clinic%0A%0A`+`الاسم: ${encodeURIComponent(d.get('name')||'')}%0A`+`رقم الموبايل: ${encodeURIComponent(d.get('phone')||'')}%0A`+`الخدمة المطلوبة: ${encodeURIComponent(d.get('service')||'')}%0A`+`التاريخ: ${encodeURIComponent(d.get('date')||'')}%0A`+`الساعة: ${encodeURIComponent(d.get('time')||'')}%0A`+`أفضل وقت للتواصل: ${encodeURIComponent(d.get('preferred')||'')}%0A`+`ملاحظات: ${encodeURIComponent(d.get('notes')||'لا يوجد')}`;window.open(`https://wa.me/201221753277?text=${msg}`,'_blank')});
